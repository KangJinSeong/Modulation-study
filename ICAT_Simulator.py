...
class Simulator:
    ...
    def Mseq_GEN(self, order, index):
        inidata = np.zeros(order)
        inidata[index] = 1
        inidata = list(inidata)
        mseq = LFSR(initstate=inidata, fpoly= self.taps)
        cyc = 2**(order)
    ...
    def SIG_GEN(self,q,r,order, index):
        ...
        bit_L = len(bit_t)
        tempseq = self.Mseq_GEN(order,index)
        print(tempseq)
    ...
    def Demod(self, coef,y, window_size):
    ...
    def mod_IQ(self,q,r,order, index):
        bit_t = np.arange(start = 0, stop = self.Tc * q, step = 1/self.Fs)
        bit_L = len(bit_t)
        tempseq = self.Mseq_GEN(order,index)
        print(tempseq)
        ...
        SIG = S_BB.real*np.cos(2*np.pi*self.Rc*symbol_t) + 1j*S_BB.imag*np.sin(2*np.pi*self.Rc*symbol_t)
        print(SIG[:10])
        y = []
        for i in range(0, r):
            y.extend(SIG)
